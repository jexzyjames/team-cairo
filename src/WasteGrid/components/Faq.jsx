...
const Faq = ({ title, content, isOpen, onClick }) => {
  return (
    <div
      id="faq"
      className=" bg-gray-100 fades rounded-md w-full   overflow-hidden mb-1  "
    >
      <button
        onClick={onClick}
      ...
      >
        ...
        <span
          ...
          className={`rounded-[200px] ${
            isOpen ? "bg-green-400 text-white" : "bg-white"
          }  text-black cursor-pointer  py-1 px-1   text-md shadow-lg `}
        >
          {isOpen ? <FaMinus /> : <FaPlus />}
        </span>
      </button>
      {isOpen && <div className="px-4 mb-2  text-gray-700 ">{content}</div>}
    </div>
  );
};
